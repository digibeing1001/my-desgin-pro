...
| 8 | 🇺🇸 OpenAI | **GPT Image 2** | ~$0.21(H) | ¥1.50 | 2K(2048×2048) | 1,506 | 超旗舰 | 🟢 在线 |
| 9 | 🇺🇸 OpenAI | **GPT Image 1.5** | $0.034(M) | ¥0.25 | 1536×1024 | 1,264 | 旗舰级 | 🟢 在线 |
| 10 | 🇺🇸 OpenAI | **GPT Image 1 Mini** | $0.011(M) | ¥0.08 | 1536×1024 | 1,200 | 草稿级 | 🟢 在线 |
| 11 | 🇩🇪 BFL | **Flux 2 Pro** | $0.03/MP | ¥0.22/MP | 4MP | 1,265 | 旗舰级 | 🟢 在线 |
...
- 中文提示词理解最佳（中文原生模型）
- 支持联网搜索融合实时信息
- **组图生成能力**：支持 `sequential_image_generation` 模式，可一次生成多张连贯图（故事板/连续场景）
- 支持PNG透明背景
- 火山方舟平台国内直连，无需翻墙
...
> ⚠️ DALL-E 2/3 已于 **2026-05-12 停服**，不再列入。

#### GPT Image 2（🏆 质量 #1 + Agentic 推理）
...
| **单张成本** | Low≈$0.04 / Medium≈$0.10 / High≈$0.21（1024×1024 参考） |
| **定价模式** | Token 计费：Text $5/M · Image input $8/M · Image output $30/M |
| **quality参数** | `low` / `medium` / `high` |
| **thinking参数** | `low` / `medium` / `high`（O-series 推理，规划后再绘制） |
| **size参数** | 灵活尺寸：最大边 ≤4000px，总像素 ≤8,294,400（约4K），两边为16的倍数，长宽比 ≤3:1 |
| **推荐尺寸** | `1024x1024` / `1792x1024` / `1024x1792` / `2048x2048` |
| **最大分辨率** | 2K 原生稳定，4K 实验性 |
| **特色** | LM Arena Elo 1506（质量 #1）、Agentic 推理、多语言文本渲染 99%+、单次最多10张 |

**✅ 优势：**
- 全球图像质量 #1（LM Arena 断层领先 +242 分）
- 首个具备 Agentic 推理的图像模型：先规划构图，再生成，复杂场景一次成功率极高
- 多语言文本渲染极强（中文/日文/韩文/印地语等字符级精确）
- 支持最多 10 张连贯图批量生成
- 2K 原生分辨率，印刷级输出可用
- **多组图生成**：单次请求最多可生成 10 张连贯图（`n: 10`），适合故事板/多角度批量产出
- 集成实时 Web 搜索，信息图/地图等需要事实准确的场景表现极佳

**❌ 劣势：**
- 国内需代理
- High 质量约 $0.21/张，比 1.5 贵约 60%
- GA 版本暂不支持透明背景（需继续用 1.5）
- `input_fidelity` 参数对 gpt-image-2 已禁用
- 4K 分辨率仍为实验性，不建议用于生产

**💡 使用建议：**
- **复杂排版/信息图/多语言物料**：优先使用 `thinking=medium`，布局准确率最高
- **概念探索/草图**：`quality=low` 即可，性价比极高
- **需要透明背景**：退回 `gpt-image-1.5`
- **印刷物料**：2K 输出可直接用于数字端；印刷需确认 300dpi 等效尺寸是否足够
...
## 七、一致性与 Seed 控制（漂移问题缓解）

> **核心事实**：扩散模型每次生成都是独立随机采样，**绝对一致性在技术上不可能**。本节提供缓解方案，但无法根治。

### 漂移根因

| 因素 | 说明 |
|------|------|
| **随机采样** | 扩散模型从噪声中采样，每次噪声初始化不同 |
| **Seed 不可控** | 多数 API 不暴露 seed，或 seed 仅控制初始噪声，模型更新后同一 seed 结果也会变 |
| **参考图权重** | reference_image 被当作"风格参考"而非"结构模板"，AI 会重绘而非精确复制 |
| **多图一致性** | 同一 prompt 生成 4 张图，4 张之间构图/元素位置都会有差异 |

### 各模型一致性能力对比

| 模型 | Seed 控制 | 组图一致性 | 参考图保真度 | 一致性评级 |
|------|:---------:|:----------:|:------------:|:----------:|
| Seedream 5.0 | ⚠️ 有限 | ✅ sequential_image_generation | ⚠️ 中等 | 🟡 中等 |
| Seedream 4.5 | ⚠️ 有限 | ✅ 组图模式 | ⚠️ 中等 | 🟡 中等 |
| Flux 2 Pro | ✅ 支持 seed | ⚠️ 同 seed 仍可能漂移 | ⚠️ 中等 | 🟡 中等 |
| GPT Image 1.5 | ❌ 不暴露 | ❌ 无组图模式 | ⚠️ 中等 | 🔴 弱 |
| Nano Banana 2 | ❌ 不暴露 | ❌ 无组图模式 | ⚠️ 中等 | 🔴 弱 |
| Ideogram 3.0 | ❌ 不暴露 | ❌ 无组图模式 | ⚠️ 中等 | 🔴 弱 |

### 缓解方案

```
方案1: 固定 Seed（仅限支持模型）
├── 适用: Flux 2 Pro / 部分 Seedream 场景
├── 效果: 同一 prompt + 同一 seed ≈ 70-85% 相似
└── 限制: 模型版本更新后同一 seed 结果可能改变；参考图存在时 seed 效果下降

方案2: 组图模式（Seedream 系列）
├── 适用: Seedream 5.0 / 4.5
├── 效果: 同批次组图一致性 > 单张独立生成
└── 限制: 仍非 100% 一致；仅适用于同场景变体

方案3: 强参考图 + 低创意权重
├── 适用: 支持 reference_image 的模型
├── 效果: 让 AI "更接近"参考图，但不会精确复制
└── 限制: 参考图被当作风格引导，结构仍会变形

方案4: 确定性工具替代（根治方案）
├── 适用: 需要像素级精确或跨图一致性的场景
├── 工具: HTML/Canvas/SVG/3D软件
└── 效果: 100% 一致性
```

### 黄金决策

> **如果用户说"和上一张保持一致"或"这套物料风格要统一"：**
> 
> 1. 首稿可用 AI 生图探索方向
> 2. 确认方向后，**立即切换到确定性工具**（Canvas/SVG/HTML）制作精确稿
> 3. AI 生图仅用于「氛围参考」或「背景纹理」，不用于「结构主体」
> 4. 需要多物料一致性时，使用**同一套预渲染素材**（R11）而非多次 AI 生图
