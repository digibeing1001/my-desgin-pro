...
### 新增
- **R23 · 最终交付物本地归档规则（R-DELIVER）**：用户最终确认、明确表示"采用"的产物，必须在 Phase 6 保存到桌面文件夹 `C:\Users\{用户名}\Desktop\{品牌名}_交付物_YYYYMMDD\`。文件夹分三层：01_源文件 / 02_预览文件 / 03_规范文档
- **R23-A · 归档即最终交付**：文件保存到桌面并告知用户路径后，才算正式完成交付。未执行本地归档 = 交付未完成

### 强化
- **Phase 6 P6-G3 最终交付门**：新增本地归档强制执行流程，包含最终确认 → 明确采用 → 归档保存三步。归档前必须获得用户"采用/满意/就这个"的明确回应，禁止保存未确认的中间稿

### 为什么做这些变更
- **触发原文**："形成用户的产出物之后，在桌面生成一个文件夹，并把用户最后确认、明确表示采用的产物全部下载存到这个文件夹中，供用户后续直接使用"
- **根因**：1) 原有流程仅列出交付物清单，未强制保存到用户本地；2) 用户可能在对话结束后找不到文件；3) 中间稿/草稿与最终确认稿未区分，容易混淆
- **修正**：R23 强制桌面归档 + R23-A 归档即交付 + Phase 6 交付门补充三步流程

### 影响
- 用户最终确认的设计文件自动保存到桌面，打开即用
- 源文件/预览文件/规范文档分层存放，便于后续修改和印刷
- 交付完成有明确标志（本地归档+路径告知），杜绝"假交付"
...
### 修正
- **R-T6 从"自动启动 Console"改为"询问用户偏好"**：Skill 被加载时，**禁止自动启动 Console**。必须先询问用户：「你想通过对话界面还是图形界面使用？」用户选择对话界面 → 继续文字对话；选择图形界面 → 才启动 Console
- **R-T7 从"自动检测启动"改为"按需启动"**：只有用户**明确输入** `design gui` / `打开console` / `gdpro` 等关键词时，才检测并启动 Console。禁止在用户未主动要求时检测或启动
- **Console 执行规则修正**：去掉"技能被加载时"的自动启动描述，改为"用户明确要求打开图形界面时"
...
- **修正**：R-T6 改为「先询问偏好，再问模型，再进工作流」；R-T7 改为「按需启动」；Console 启动完全由用户主动触发

### 影响
- 用户打开 skill 后不再被强制跳转到图形界面，尊重用户的使用习惯
- 对话界面用户可以快速开始设计沟通，需要可视化时再输入 `design gui` 切换
- 图形界面用户仍可随时通过关键词启动 Console，体验不受损
...
### 修正
- **R-IM0 启动即配置**：明确「无论是否已配置，都必须在本项目/本对话开始时询问用户选择默认生图模型」。上次选择仅作为推荐项展示，必须让用户**明确确认**，不得自动沿用
- **新增 R-IM0-A · 项目内模型默认延续规则**：本项目第一次选择模型后，后续所有生图默认使用该模型。只有用户**明确说"切换模型"**时才重新展示列表。**禁止在用户未要求切换的情况下擅自更换模型**。每个新项目/新对话开始时，R-IM0 重新触发
- **R-IM4 更新**：补充说明默认行为是延续首次选择，只有用户主动要求时才切换
- **一致性自检清单**：新增「本项目是否已确认默认生图模型？当前生图是否使用了用户未要求切换的模型？」

### 为什么做这些变更
- **触发原文**："启动时是不是会先询问用户调用哪个模型...后面一直延续第一次选择的模型...每个对话框中同一个项目的第一次选择...新项目还是要再次询问"
- **根因**：1) 原 R-IM0 允许"直接采用上次选择"，可能导致新项目自动沿用旧模型而不询问；2) 缺少"项目内默认延续、禁止擅自切换"的硬规则；3) 用户需要明确的预期管理：选一次，锁全程，新项目再问
- **修正**：R-IM0 改为「必须询问+确认」；新增 R-IM0-A 锁定项目内模型；R-IM4 补充默认延续说明

### 影响
- 用户在新项目中一定会被问到模型选择，不会被悄悄沿用旧设置
- 项目内模型稳定，不会因为执行者"觉得另一个模型更好"而擅自变更
- 模型切换完全由用户主动发起，预期管理清晰
...
### 新增
- **R22 · 决策辅助强制规则（R-ADVISOR）**：当用户无法明确材质/工艺/品类（回答"不知道""你推荐""随便"）时，禁止跳过或直接替用户决定。必须进入顾问模式，读取 `references/material-guide.md`，提供 2–3 个选项（含适用理由+优点+局限+参考价格），等用户明确选择后方可继续
- **`references/material-guide.md`**：按品类（食品/化妆品/数码3C/礼品/服装/印刷物料/招牌）的材质、工艺、市场参考价格完整指南。覆盖经济型/标准型/高端型三档方案，含 UV/烫金/覆膜/凹凸等工艺速查表
- **R-IM10 · 三种生图方式强制声明（R-THREE-MODES）**：每次生图前必须向用户声明三种方式并让其选择：①文生图 ②图生图 ③多组图生成（批量连贯图）

### 强化
- **image-models.md**：Seedream 5.0 Lite 和 GPT Image 2 补充多组图生成能力说明
- **Phase 1 Layer 2 执行规则**：增加顾问模式触发流程（R22）
- **参考资料体系**：新增 `material-guide.md` 索引

### 为什么做这些变更
- **触发原文**："追问必须依据不同品类制定" / "如果用户无法明确回答，不要直接动手" / "可以提供 UV 材质建议，说明好处和价格" / "内容生成提供三种方式：文生图、图生图、多组图生成"
- **根因**：1) 原有流程缺少按品类的材质/工艺决策支持，用户说"不知道"时执行者容易直接替用户决定；2) 缺少市场价格参考，导致用户拿到方案后才发现超预算或材质不合适；3) 用户不知道除了文生图还有图生图和多组图生成选项
- **修正**：新增 R22 + material-guide.md（5大品类×3档方案+工艺速查）+ R-IM10（三种生图方式声明）

### 影响
- 材质/工艺选择从"靠猜"变成"有据可依"，用户决策质量大幅提升
- 用户获得包含理由和价格的选项，预算匹配度更高
- 生图方式从单一文生图扩展为三种，产品渲染/批量产出更灵活
...
### 新增
- **🟣 产品渲染模式**：当需求涉及产品效果图/电商主图/详情页/包装产品图/3D渲染时，触发专用工作流
- **产品类设计追问清单（15问）**：融入自 `product-demo-generator`，覆盖参考图预检、产品身份、物理形态、材质颜色、细节品牌、场景风格、一致性输出。追问完成后锁定 PIF（Product Identity Fingerprint）
- **R-IM9 · 产品渲染一致性链路（R-CHAIN）**：同一产品的多视角/多场景输出，首图确认后，后续所有图必须以首图为参考图生成，PIF 逐字完整引用，scale 建议 0.3–0.4，最多 3–5 张/链路
- **关键原则第24-25条**：「产品先访谈，后渲染」「一致性链路保产品」

### 强化
- **Phase 1 资产预检**：增加参考图特殊处理规则（产品渲染模式），上传参考图后必须描述确认、提取视觉特征、登记到 PIF
- **Phase 1 Layer 2 追问维度**：新增「产品类设计」维度，明确产品渲染模式必触发 15 问

### 为什么做这些变更
- **触发原文**："帮我融入 product-demo-generator skill" / "前置询问和过程中的追问要明确帮用户把需求描述清楚" / "产物必须能在 Photoshop 中后期编辑"
- **根因**：1) my-desgin 原有追问法面向品牌/平面，缺少产品类（外形/材质/结构/表面处理）的系统化访谈；2) 产品多视角渲染时无一致性保障 = 同产品不同视角长得不一样；3) product-demo-generator 的 PIF + 一致性链路方法论成熟，可直接融入
- **修正**：模式路由新增产品渲染模式 + 15问产品访谈 + PIF 锁定 + 一致性链路规则

### 影响
- 产品效果图/电商主图/包装产品图的需求得到专业化处理
- 同一产品的多视角渲染保持外观一致性，电商/包装可直接使用
- PIF 锁定机制防止产品特征漂移
...
### 新增
- **R-IM0 · 启动即配置**：Skill 被加载时立即检测已配置的 API 密钥，若无任何配置则立即暂停并引导用户选择默认生图模型，禁止未配置直接进入工作流
- **R20 · HTML 效果图定位规则（R-HTML）**：明确 HTML/CSS/Canvas 渲染图是「精确排版参考稿」，绝对不能直接用于印刷或作为最终交付物。印刷物料必须在 HTML 确认后在 AI/PS/ID 中重绘并添加完整印刷标记
- **R21 · 印刷标示线强制规则（R-PRINT-MARKS）**：所有实体印刷交付物必须包含完整印刷标示系统：出血线(3mm) + 裁切标记 + 套印标记 + 颜色条 + 模切/折叠线
- **GPT Image 2 模型**：加入 OpenAI gpt-image-2（2026-04-21 发布），含完整 API 参数、定价、分辨率、thinking 模式说明

### 强化
- **印刷检查清单**：从 5 项扩展为 8 项，新增裁切标记、套印标记、颜色条，并附 HTML 截图不可直接印刷的警告
- **刀图规范**：增加裁切标记、套印标记、颜色条，明确模切线/折线的颜色标注（红/蓝）
- **模型总览**：10家16+ → 10家17+

### 为什么做这些变更
- **触发原文**："启动时先让用户配置生图模型 API" / "HTML 的效果图可以拿去后期修改或印刷吗？" / "刀图一定要有标示线"
- **根因**：1) 生图模型到用的时候才问 = 流程中断；2) HTML 截图被误当作可印刷交付物 = 严重交付事故；3) 刀图缺少专业印刷标示线 = 印刷厂无法准确生产
- **修正**：启动即配置（R-IM0）+ HTML 明确定位（R20）+ 印刷标示线铁律（R21）+ GPT Image 2 加入

### 影响
- 用户不再会在设计中途才发现没配 API
- HTML 中间稿和印刷交付物的界限被严格区分，杜绝"截图直接印刷"的事故
- 包装/印刷物料的生产可用性大幅提升，印刷厂可直接使用
