...
### 新增
- **R19 · 交付物源文件强制规则（R-SOURCE）**：所有设计交付物必须同时提供①可编辑源文件（Photoshop .PSD / Illustrator .AI/.EPS / Sketch .Sketch / Figma .fig 等保留完整图层的原生格式）和②预览/印刷文件（PNG/JPG/PDF）。禁止仅以位图、AI生成图或HTML截图作为唯一交付物
- **源文件自检清单项**：一致性自检清单新增「本物料是否已提供可编辑源文件（PSD/AI/EPS/Sketch/Figma），且图层/文字/矢量路径未栅格化」
- **description 扩展**：`AI/EPS` 加入支持格式列表，与原有的 SVG/PSD 并列

### 强化
- **关键原则第14条**：从"矢量/PSD可编辑"升级为"矢量/PSD/AI/EPS可编辑源文件是交付底线"，措辞更硬

### 为什么做这些变更
- **触发原文**："最后交付物中必备包含Photoshop、Illustrator等可以编辑修改的矢量图"
- **根因**：Skill 虽有"输出实用"原则，但缺乏硬规则级约束，执行者可能以预览图/AI生成图/HTML截图代替可编辑源文件交付，导致用户无法二次修改、无法印刷落地
- **修正**：将"可编辑源文件"从建议升级为铁律（R19），明确列出 AI/EPS/PSD/Sketch/Figma 等格式要求，并在自检清单中强制检查

### 影响
- Phase 4-6 的交付环节必须同时产出源文件+预览文件，缺一不可
- 用户获得的设计文件可真正用于二次修改、印刷生产和VI延展
- 违反 R19 的交付视为作废，必须补全源文件后方可通过验收
